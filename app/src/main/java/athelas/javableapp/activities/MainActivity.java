...
/**
 * Sets up a 2-way blue tooth connection for a given device.
 */
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    private static final String TAG = "MainActivity";

    // the UUID of your bluetooth device (I used an HC02 Bluetooth module for arduino)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Bluetooth Device Pairing");
        ...
        // Steps for connecting to bluetooth:
        // Turn Bluetooth on
        Button btnOnOff = (Button) findViewById(R.id.enableDisableBT);

        // Make sure your phone is discoverable for other devices
    ...
    public ImageButton toRecievingPageBtn;

    public void onCreateActSwitch() {
        toRecievingPageBtn = (ImageButton) findViewById(R.id.btToVitalsBtn);
        toRecievingPage(false);

        toRecievingPageBtn.setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
                Log.d(TAG, "onCreateActSwitch: switching to LiveVitalsActivity");
                Intent loadNewAct = new Intent(MainActivity.this, ReceiveDataActivity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            Log.d(TAG, "onActivityResult: returned from Receiving Page");
            // do anything from the LiveVitalActivity
        } else if (resultCode == RESULT_CANCELED) {
            Log.d(TAG, "onActivityResult: returned from failed Receiving Page");
            Utils.toastMessage(getApplicationContext(), "Receiving Page Failed");
            toRecievingPage(false);
            btnStartConnection.setEnabled(false);
        }
    }

    private void toRecievingPage(boolean enabled) {
        toRecievingPageBtn.setEnabled(enabled);
        toRecievingPageBtn.setColorFilter(Color.argb((enabled ? 0 : 255), 154, 154, 154));
    }
